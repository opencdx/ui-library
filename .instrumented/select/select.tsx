...
function Select<T extends object>(props: Props<T>, ref: ForwardedRef<HTMLSelectElement>) {
  ...
  const renderIndicator = useMemo(() => {
    const spinnerRef = React.createRef<HTMLElement | null>();

    if (isLoading) {
      return <Spinner ref={spinnerRef} />;
    }

    return clonedIcon;
  }, [isLoading, clonedIcon]);

  const popoverContent = useMemo(
    () =>
      state.isOpen ? (
        <FreeSoloPopover {...getPopoverProps()}>
          <ScrollShadow {...getListboxWrapperProps()} ref={React.createRef<HTMLElement | null>()}>
